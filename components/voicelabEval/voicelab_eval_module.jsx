...
// TPS = 저장된 시점 라벨(과거 mid 세션 표시용이라 유지). TP_CHOICES = 지금 고를 수 있는 시점.
const TPS = { pre: "사전", mid: "중간", post: "사후" };
const TP_CHOICES = ["pre", "post"];
...
// =================================================================
// 모듈 순서 정의 — 사전(pre) · 사후(post) 공통.
//   중간점검(mid)은 아직 설계 전이라 선택지에서 뺐다(TP_CHOICES). 저장 포맷·보고서는
//   mid 를 계속 지원하므로, 만들 때 TP_CHOICES 에 "mid" 를 되넣고 전용 목록을 두면 됨.
...
const ACTIVE_MODULES = MODULES_FULL.filter((m) => !m.off);
      ...
      <Shell sub="지산학 사업 · 인지·언어·정서·삶의 질 평가">
        <Card>
          ...
          <div className="flex gap-2 mb-2">
            {TP_CHOICES.map((k) => <Pill key={k} active={timepoint === k} onClick={() => setTimepoint(k)}>{TPS[k]}</Pill>)}
          </div>
